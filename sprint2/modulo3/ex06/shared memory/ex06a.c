...
int main(void) {
	...
	int i;
	const int DATA_SIZE = sizeof(Valores);
    ...
    if (id == 0) { //Código do pai
		...
		Valores* shared_data = (Valores*) mmap(NULL, DATA_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
		...
		Valores* shared_data = (Valores*) mmap(NULL, DATA_SIZE, PROT_READ, MAP_SHARED, fd, 0);
		if(shared_data == MAP_FAILED) {
			perror("failed mmap!!!\n");
			exit(EXIT_FAILURE);
		}
